...
folder = '/data_KTP/승용_자율주행차_데이터셋'

# 데이터셋 폴더 안에 있는 라벨링 파일 리스트 불러오기
file_list = glob.glob(folder + "/label/*.json",
                      recursive=True)
print(file_list)
# 변환된 값들을 저장하는 폴더 생성(폴더 존재하면 다시 안만듬)
text_folder = folder + "/label_to_text"
...
for file in file_list:
    ...
    with open(text_file_path, "w") as text_file:
        for annotation in annotations:
            ...
            # 카테고리 아이디가 차선인지 확인
            if 'Lane' in annotation['class'] :
                # 세그맨테이션 항목을 따로 저장.
                seg = annotation['polygon']
                set_data.append(annotation['class'])
