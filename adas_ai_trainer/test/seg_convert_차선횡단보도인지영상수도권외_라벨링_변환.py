...
# 사용한 데이터셋 : 차선횡단보도인지영상수도권외
# 데이터셋이 들어간 폴더 경로. 지정해줄것. 2660541.jpg 형식임.
json_folder = '/home/kgj0096/라벨링데이터/1280_720'
img_folder = '/home/kgj0096/원천데이터/1280_720'

# 변환된 값들을 저장하는 폴더 생성(폴더 존재하면 다시 안만듬). 그림 만들때 씀
txt_folder = "./contours"
os.makedirs(txt_folder, exist_ok=True)
text_folder = "./yolo_seg/labels/train/차선횡단보도인지영상수도권외_라벨링_변환"
...
for json_file, jpg_file in zip(json_file_list, jpg_file_list):
    ...
    for annotation in data['annotations']:
        ...
        if annotation['class'] in ['traffic_lane']:
            segmentation = annotation['data']
            seg = segmentation

            # [(x1, y1), (x2, y2), ...] 형식으로 변환
            points = [(seg[i]['x'], seg[i]['y'])
